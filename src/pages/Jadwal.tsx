...
interface SchedulesProps {
  ...
  onNavigateNotification?: () => void; // <--- TAMBAHAN PROPS NOTIFIKASI
}
...
export default function Schedules({
  ...
  onNavigateNotification // <--- TERIMA PROPS DI SINI
}: SchedulesProps) {
  ...
  const [selectedDay, setSelectedDay] = useState<string>('Senin'); // Tetap simpan dlm B.Indo sbg ID
  ...
  // Form States (Pelajaran)
  const [formSubject, setFormSubject] = useState('');
  const [formStartTime, setFormStartTime] = useState('');
  const [formEndTime, setFormEndTime] = useState('');
  const [formDay, setFormDay] = useState('Senin'); // Tetap simpan dlm B.Indo
  const [formRoom, setFormRoom] = useState('');

  // Form States (Kegiatan)
  ...
  return (
    <div className={`bg-[#eef5f7] dark:bg-[#1a1c1e] min-h-screen flex items-center justify-center p-4 transition-colors duration-300`}>
      ...
      <div className="bg-[#f8f9fa] dark:bg-[#1a1c1e] text-[#161d1f] dark:text-[#e2e2e5] min-h-screen sm:min-h-[800px] h-full sm:h-[800px] w-full max-w-md sm:rounded-[2.5rem] sm:shadow-2xl flex flex-col font-body-main antialiased relative overflow-hidden pb-20 transition-colors duration-300">
        ...
        {/* Global Click-Away Listener Overlay untuk Dropdown */}
        {activeMenuId && (
          <div className="fixed inset-0 z-30" onClick={() => setActiveMenuId(null)}></div>
        )}

        {/* HEADER */}
        <header className="flex justify-between items-center px-[20px] py-[16px] w-full bg-[#f8f9fa] dark:bg-[#1a1c1e] sticky top-0 z-20 border-b border-[#e9ecef]/50 dark:border-[#44474e]/50 transition-colors duration-300">
          <div className="flex items-center gap-[8px]">
            <button onClick={onNavigateHome} className="hover:bg-[#eef5f7] dark:hover:bg-[#44474e] transition-colors rounded-full p-1 text-[#005da7] dark:text-[#a4c9ff] flex items-center justify-center">
          ...
          <div className="flex items-center gap-1">
            <button onClick={onNavigateNotification} className="hover:bg-[#eef5f7] dark:hover:bg-[#44474e] transition-colors rounded-full p-2 text-[#005da7] dark:text-[#a4c9ff] flex items-center justify-center" aria-label="Notifikasi">
        ...
        {/* MAIN CONTENT */}
        <main className="flex-grow w-full overflow-y-auto no-scrollbar px-[20px] py-[20px] flex flex-col gap-[20px]">
              ...
              <section>
                <div className="flex gap-[8px] overflow-x-auto no-scrollbar pb-1 snap-x">
                  {DAYS_DB.map((day) => (
                    ...
                    >
                      {/* Tampilkan hari sesuai bahasa, simpan sebagai B.Indo di state */}
      ...
      {/* Tombol Floating Action (FAB) Tambah Jadwal */}
<button 
  onClick={() => { resetForms(); setIsModalOpen(true); }} // Atau sesuaikan dengan fungsi klik di Jadwal kamu
...
>
  <span className="material-symbols-outlined text-[24px]">add</span>
        ...
        <nav className="absolute bottom-0 w-full rounded-t-2xl bg-[#ffffff] dark:bg-[#1a1c1e] shadow-[0px_-4px_20px_rgba(0,0,0,0.05)] z-20 flex justify-around items-center px-[8px] py-[12px] border-t border-[#e9ecef] dark:border-[#44474e] transition-colors duration-300">
          ...
          <button onClick={onNavigateSchedule} className="flex flex-col items-center text-[#005da7] dark:text-[#a4c9ff] bg-[#d4e3ff]/50 dark:bg-[#004883]/50 rounded-xl px-4 py-2 transition-colors">
              ...
              {activeTab === 'pelajaran' ? (
                <form onSubmit={handleSubmitSchedule} className="flex flex-col gap-3">
                  <input
                    ...
                    className="w-full p-3 rounded-xl border border-[#c1c7d3] dark:border-[#44474e] bg-[#ffffff] dark:bg-[#1a1c1e] text-[#161d1f] dark:text-[#e2e2e5] focus:outline-[#005da7] text-sm transition-colors"
                  />
                  <div className="flex gap-3">
                    <div className="w-1/2 flex flex-col gap-1">
                      ...
                      <input
                        type="time" required
                        value={formStartTime} onChange={(e) => setFormStartTime(e.target.value)}
                        className="w-full p-3 rounded-xl border border-[#c1c7d3] dark:border-[#44474e] bg-[#ffffff] dark:bg-[#1a1c1e] text-[#161d1f] dark:text-[#e2e2e5] focus:outline-[#005da7] text-sm transition-colors"
                      />
                    </div>
                    <div className="w-1/2 flex flex-col gap-1">
                  ...
                  <div className="flex gap-3">
                    <select
                      ...
                      className="w-1/2 p-3 rounded-xl border border-[#c1c7d3] dark:border-[#44474e] bg-white dark:bg-[#1a1c1e] text-[#161d1f] dark:text-[#e2e2e5] focus:outline-[#005da7] text-sm transition-colors"
                    >
                      {DAYS_DB.map(d => (
                        <option key={d} value={d}>
                          {t.days[d as keyof typeof t.days]}
                        </option>
                      ))}
                    </select>
                    <input
                      type="text" placeholder={t.room_placeholder}
                      value={formRoom} onChange={(e) => setFormRoom(e.target.value)}
                      className="w-1/2 p-3 rounded-xl border border-[#c1c7d3] dark:border-[#44474e] bg-[#ffffff] dark:bg-[#1a1c1e] text-[#161d1f] dark:text-[#e2e2e5] focus:outline-[#005da7] text-sm transition-colors"
                    />
                  </div>
                ...
                <form onSubmit={handleSubmitEvent} className="flex flex-col gap-3">
                  <input
                    ...
                    value={formEventTitle} onChange={(e) => setFormEventTitle(e.target.value)}
                    className="w-full p-3 rounded-xl border border-[#c1c7d3] dark:border-[#44474e] bg-[#ffffff] dark:bg-[#1a1c1e] text-[#161d1f] dark:text-[#e2e2e5] focus:outline-[#005da7] text-sm transition-colors"
                  />
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      type="date" required
                      value={formEventDate} onChange={(e) => setFormEventDate(e.target.value)}
                      className="w-full p-3 rounded-xl border border-[#c1c7d3] dark:border-[#44474e] bg-[#ffffff] dark:bg-[#1a1c1e] text-[#161d1f] dark:text-[#e2e2e5] focus:outline-[#005da7] text-sm transition-colors"
                    />
                  </div>
                  <div className="flex gap-3">
                    <div className="w-1/2 flex flex-col gap-1">
                      ...
                      <input
                        type="time" required
                        value={formEventStartTime} onChange={(e) => setFormEventStartTime(e.target.value)}
                        className="w-full p-3 rounded-xl border border-[#c1c7d3] dark:border-[#44474e] bg-[#ffffff] dark:bg-[#1a1c1e] text-[#161d1f] dark:text-[#e2e2e5] focus:outline-[#005da7] text-sm transition-colors"
                      />
                    </div>
                    <div className="w-1/2 flex flex-col gap-1">
                      ...
                      <input
                        type="time" required
                        value={formEventEndTime} onChange={(e) => setFormEventEndTime(e.target.value)}
                        className="w-full p-3 rounded-xl border border-[#c1c7d3] dark:border-[#44474e] bg-[#ffffff] dark:bg-[#1a1c1e] text-[#161d1f] dark:text-[#e2e2e5] focus:outline-[#005da7] text-sm transition-colors"
                      />
                    </div>
                  </div>
                  <input
                    type="text" placeholder={t.location_placeholder}
                    value={formEventLocation} onChange={(e) => setFormEventLocation(e.target.value)}
                    className="w-full p-3 rounded-xl border border-[#c1c7d3] dark:border-[#44474e] bg-[#ffffff] dark:bg-[#1a1c1e] text-[#161d1f] dark:text-[#e2e2e5] focus:outline-[#005da7] text-sm transition-colors"
                  />
                  <button type="submit" className="w-full mt-2 py-3 bg-[#005da7] text-white font-bold rounded-xl active:scale-95 transition-all shadow-md">
